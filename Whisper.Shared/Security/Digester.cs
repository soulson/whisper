...
namespace Whisper.Shared.Security
{
    ...
    {
        private readonly ILog log = LogManager.GetLogger(typeof(Digester));
        ...
        public Digester(HashAlgorithm hashAlgorithm)
        {
            ...
            log.DebugFormat("digester initialized with {0}{1}algorithm of output size {2}", algorithm.CanReuseTransform ? "reusable " : "", algorithm.CanTransformMultipleBlocks ? "multi-block " : "", DigestSize);
        }
        ...
        public virtual byte[] CalculateDigest(byte[] input)
        {
            algorithm.Initialize();
            return algorithm.ComputeHash(input, 0, input.Length);
        }

        public virtual byte[] CalculateDigest(String message, Encoding encoding)
        {
        ...
        public virtual byte[] CalculateDigest(params BigInteger[] bigInts)
        {
